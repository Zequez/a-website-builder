...
const i18n = {
  en: {
    heroTitle: (
      <>
        An <strong>open web creation platform</strong>
      </>
    ),
    heroSub:
      'Hoja is a members-driven project that aims to bring us together in creating our own web and apps with shared tools and knowledge',
  },
  es: {
    heroTitle: (
      <>
        Una <strong>plataforma abierta de creación web</strong>
      </>
    ),
    heroSub:
      'Hoja es un proyecto guiado por sus miembros con el fin de unirnos para crear nuestras propias páginas web y aplicaciones con herramientas y conocimiento compartido',
  },
};
...
const ValuePropositionPage = () => {
  ...
  return (
    <div class="flex flex-col bg-gray-100 text-black/80 min-h-screen">
        ...
        <div class="relative z-20 flex max-w-screen-md mx-auto text-white">
          <div class="flexcc flex-col h-full . xs:w-6/8 sm:w-1/2 px-2 . xs:px-4  py-2 sm:py-8 md:py-16 lg:py-24">
            <div class="bg-black/30 p-4 rounded-md">
              <h2 class="text-lg sm:text-2xl mb-2 line-height-tight">{t('heroTitle')}</h2>
              <p class="opacity-75 text-sm sm:text-base">{t('heroSub')}</p>
            </div>
          </div>
        </div>
        <img
      ...
      <MembersExplorer class="max-w-screen-md mx-auto mt-16" />
      {/* <JoiningShowcase /> */}
      <ProgressiveSignUp />
    </div>
  );
